...
import { Endata, Dedata } from '../../surgeryList/NHSSurgery_data';
...
export default function NHSNewRequesstForm() {
  ...
  const addnewRequest =(value)=>{
    axios({
      method: "POST",
      url: "https://meindoc.app/backend/api/NHS_new_request.php",
      data: value,
      headers: {
     ...
     setResponse(response.data.message);
      if(response.data.success === 1 && response.data.result){
        const request ={
          ...
          requestid:response.data.result
        }
        axios({
           method: "POST",
           url:"https://meindoc.app/backend/mailservices/NHS_mail.php",
...
// Assigning values to add request to the server
  const  makeNewRequest = async (values) =>{
    ...
    const response = await addnewRequest(JSON.stringify(data)); 
    return response;
  }
